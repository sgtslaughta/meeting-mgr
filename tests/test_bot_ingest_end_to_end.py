...
CHUNK_COUNT = 10  # cross ten: "10" sorts before "9" lexically, so <=9 chunks
...
def test_normalize_reads_a_bot_written_manifest_raw_key_unmodified(monkeypatch):
    ...
    must pass with zero changes to pipeline/normalize.py. Ten real,
    distinct audio chunks (not fewer) so an out-of-order reassembly would
    corrupt the reconstructed audio badly enough for ffprobe's duration
    read to plausibly fail or drift outside tolerance.

    Kill: deleting the "manifest:" branch in normalize.py (or reordering
    _bot_chunk_seq's sort back to lexical) breaks this."""
