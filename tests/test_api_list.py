...
def test_list_returns_meetings_newest_first(monkeypatch):
    ...
    c = TestClient(app)
    first = c.post("/meetings", data={"title": "older"},
                   files={"file": ("a.m4a", b"A", "audio/mp4")}).json()["meeting_id"]
    second = c.post("/meetings", data={"title": "newer"},
                    files={"file": ("b.m4a", b"B", "audio/mp4")}).json()["meeting_id"]
    ...
    assert set(row) == {"id", "title", "status", "current_stage",
                        "failed_stage", "created_at"}
