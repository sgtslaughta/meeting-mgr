...
from meeting_mgr.pipeline.attribute import attribute, render_transcript, AttributionProposal

def _seed(mid):
    with get_session() as s:
        c = SpeakerCluster(meeting_id=mid, label="SPEAKER_00", spans=[])
        s.add(c); s.flush()
        s.add(Segment(meeting_id=mid, cluster_id=c.id,
                      start_seconds=0.0, end_seconds=1.0, text="thanks Sarah"))
