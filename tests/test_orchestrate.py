...
def test_pipeline_runs_all_stages_and_publishes(monkeypatch, make_meeting):
    ...
    monkeypatch.setattr(
        orch,
        "STAGES",
        [(name, lambda m, n=name: called.append(n)) for name in ("normalize", "diarize")]
...
def test_pipeline_publishes_transitions_and_clears_current_stage(monkeypatch, make_meeting):
    from meeting_mgr.models import Meeting

    events = []
    monkeypatch.setattr(
        orch, "publish_progress", lambda mid, stage, state: events.append((stage, state))
    )
