...
def _valid_segment_ids(meeting_id: int) -> set[int]:
    with get_readonly_session() as s:
        return {
            sid for (sid,) in s.query(Segment.id).filter(Segment.meeting_id == meeting_id).all()
        }
...
def extract_key_topics(meeting_id: int) -> None:
    ...
    with get_session() as s:
        for t in out.topics:
            ...
            s.add(
                KeyTopic(
                    meeting_id=meeting_id, title=t.title, citations=citations, provenance="inferred"
                )
            )
...
def extract_minutes(meeting_id: int) -> None:
    ...
    with get_session() as s:
        for m in out.minutes:
            ...
            s.add(
                Minute(
                    meeting_id=meeting_id, text=m.text, citations=citations, provenance="inferred"
                )
            )
