...
from meeting_mgr.models import (ActionItem, Attribution, DecisionPoint,
                                KeyTopic, Meeting, Minute, SpeakerCluster)
from meeting_mgr.participants import resolve_participant
from meeting_mgr.pipeline.app import celery_app
from meeting_mgr.pipeline.extract import (extract_action_items,
                                          extract_decision_points,
                                          extract_key_topics, extract_minutes)
...
_ARTIFACTS = {
    ...
    "minutes": (Minute, {"text"}),
    "action_items": (ActionItem, {"text", "status", "due_date",
                                  "participant_name"}),
...
def edit_artifact(meeting_id: int, artifact_type: str, item_id: int,
                  body: dict):
    with get_session() as s:
...
@router.delete("/meetings/{meeting_id}/{artifact_type}/{item_id}",
               status_code=204)
...
@router.post("/meetings/{meeting_id}/regenerate/{artifact_type}",
             status_code=202)
