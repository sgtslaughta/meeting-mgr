...
from meeting_mgr.storage import RangeNotSatisfiable, ensure_bucket, open_object, put_stream
...
_FIELDS = {
    ...
    ActionItem: ("id", "text", "participant_id", "due_date", "status", "citations", "provenance"),
    DecisionPoint: ("id", "text", "settled", "positions", "citations", "provenance"),
...
def stream_events(meeting_id: int, account: Account = Depends(get_current_account)):
    ...
    def events():
        ...
        for event in subscribe(meeting_id):
            ...
            finished_publish = event.get("stage") == "publish" and event.get("state") == "finished"
...
def read_audio(meeting_id: int, request: Request, account: Account = Depends(get_current_account)):
    ...
    try:
        stream, content_range, length = open_object(key, _valid_range(request.headers.get("range")))
